...
attachment_dir = 'C:/Users/teja/Desktop/'
...
def convert_special_char(text):
    temp=text
    special_chars = ['attherate','atred','atthered','dot','underscore','dollar','hash','star','plus','minus','space','dash']
    for character in special_chars:
        while(True):
            ...
            if pos == -1:
                ...
                elif character == 'atred':
                    temp=temp.replace('atred','@')
                elif character == 'atthered':
                    temp=temp.replace('atthered','@')
                elif character == 'dot':
...
def options_view(request):
    ...
    if request.method == 'POST':
        ...
        i = i + str(1)
        while(flag):
            texttospeech("To compose an email say create. To open Inbox folder say Inbox. To open Sent folder say Sent. To open Trash folder say Trash. To Logout say Logout. Do you want me to repeat?", file + i)
            i = i + str(1)
            say = speechtotext(3)
            if say == 'No' or say == 'no':
                flag = False
